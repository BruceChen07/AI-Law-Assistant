...
def test_call_with_fallback_retries_cloud_on_exception():
    ...
    class FakeLLM:
        def chat_with_profile(self, messages, task_profile, overrides=None):
            ...
            if payload["overrides"].get("_model_role") == "main":
                return '{"ok": true}', {"_route": {"selected_role": "main"}}
            raise RuntimeError("local model timeout")

    cfg = {"local_llm": {"allow_small_to_main_fallback": True, "execution": {"fallback_on_error": True}}}
    ...
    assert text == '{"ok": true}'
    assert raw["_route"]["selected_role"] == "main"
    assert meta["fallback_used"] is True
    assert meta["fallback_reason"] == "error"
    assert len(calls) == 2


def test_call_with_fallback_retries_main_on_invalid_result():
    calls = []
    ...
    class FakeLLM:
        def chat_with_profile(self, messages, task_profile, overrides=None):
            ...
            calls.append(payload)
            if payload["overrides"].get("_model_role") == "main":
                return '{"label":"compliant"}', {"_route": {"selected_role": "main"}}
            return 'not-json', {"_route": {"selected_role": "small"}}

    cfg = {"local_llm": {"allow_small_to_main_fallback": True, "execution": {"fallback_on_invalid_json": True}}}
